...
*   Generate a locally administered MAC address per VM instead of using the
    current fixed address, and define its lifetime across restart.
*   Bound pending receive frames with an explicit drop policy. Preserve pending
    receive behavior for startup and transient receive-ring gaps.
*   Drop a valid frame that cannot fit a posted receive buffer without failing
    the VM. Continue to reject malformed guest descriptors and invalid VirtIO
    requests.
*   Investigate carrier-status updates and configuration-change interrupts as a
    separate feature before changing current carrier behavior.
*   Test startup races, ring exhaustion, reset, multiple browser VMs, DHCP, ARP,
    and sustained bidirectional traffic through the Rust WASM adapter.

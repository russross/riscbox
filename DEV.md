...
#### External-device handoff and 9p latency
...
Make host attention an explicit CPU exit reason. C and Rust finish all device
work they can handle synchronously; an MMIO operation returns to JavaScript
only when external work is needed. Finish the guest instruction and release the
exclusive C timeslice borrow before exposing host actions. Keep VirtIO queues,
descriptors, interrupts, and pending requests in Rust. Do not add a JavaScript
crossing for ordinary MMIO or move 9p filesystem semantics into Rust.

Let the JavaScript platform driver own a bounded guest-turn cycle budget. Pass
the remaining budget into WASM and receive cycles consumed plus an exit reason:
budget exhausted, WFI, or host attention. On host attention, drain the queued
actions, start their JavaScript handlers, and resume the unused budget after
the handoff. The existing 200,000-cycle blocks do not expose actions or update
host time between calls; replace their fifteen-call loop with one C run that
can exit early for WFI or host attention. Keep the full turn bounded at the
current three million cycles, and schedule the next turn separately.

Keep each 9p session's reply API promise-only. The adapter invokes the server
when it drains a request action and attaches a completion handler to that
request's promise; neither server results nor callbacks reenter a running WASM
call. The server may resolve several outstanding request promises while
handling one new request. Preserve their independent completion, flush, reset,
and endpoint-generation semantics.

Give each request a single-shot `expectResponse` callback. A server calls it
only after determining that this request can complete without blocking I/O,
and before starting any promise or microtask chain needed to produce its
outcome. A new request may enable several earlier responses; the server calls
the callback retained for each affected request. HTTP, local-storage, and
other genuinely asynchronous work must not issue this hint. The hint carries
no result and does not enter WASM. Promise settlement remains the sole source
of reply or suppression outcomes. Account for hints by request and endpoint
generation so an unrelated completion cannot satisfy one, and retire them on
completion, reset, or close.

After dispatch, the platform driver yields to the microtask queue while hinted
responses remain outstanding. Apply settled outcomes to the retained VirtIO
queues before resuming the CPU's unused budget, then repeat as needed. Cap this
wait at 20 yields per handoff: reaching the cap indicates a likely server bug
or an unexpectedly deep promise chain, so log the unresolved hints and resume
without failing the VM. A later outcome still completes normally. Requests
without hints do not enter this wait loop. The hint is a narrow scheduling
promise, not a synchronous reply path or an attempt to inspect the browser's
microtask queue.

When any outcome settles after the guest has resumed, apply it and wake the
guest immediately. Replace any pending WFI sleep timer and coalesce wake
requests so the completion is not delayed by the previous timer or followed by
a duplicate run. Keep this driver and transport boundary generic enough to
serve a future JavaScript HTTP-backed block server through the same
host-attention, promise-completion, and wake pattern.

Detailed planning still needs the safe C exit point, run-result ABI, exact
cycle accounting, action and completion ownership across resets, and the
scheduler contract for custom hosts. Validate resident replies and multiple
responses from one request within the same browser turn, HTTP initiation before
CPU resumption, concurrent and cancelled requests, the 20-yield fault path,
WFI wakeup, and real WASM/Chrome guest behavior.
